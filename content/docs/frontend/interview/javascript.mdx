...
## 실행 컨텍스트와 이벤트 루프를 통한 비동기 처리과정이 어떻게 되나요?

JavaScript의 **실행 컨텍스트(Execution Context)** 와 **이벤트 루프(Event Loop)** 는 비동기 코드가 어떻게 실행되는지를 이해하는 핵심 개념입니다.

<Callout type="info" title="실행 컨텍스트란?">

실행 컨텍스트는 JavaScript 코드가 실행되는 환경을 의미합니다. 변수, 함수, 스코프 정보 등을 관리하며, Call Stack에서 관리됩니다.
...
<Callout type="error" title="이벤트 루프 처리 순서">
...
이벤트 루프의 이해는 JavaScript의 비동기 처리, 성능 최적화, 디버깅에 있어서 매우 중요한 개념입니다.
