...
## Typescript에서 infer는 어떻게 동작하나요?

TypeScript의 `infer` 키워드는 **조건부 타입(Conditional Types)** 내에서 타입을 추론하고 저장하는 강력한 기능입니다.

<Callout type="info" title="infer 키워드의 기본 개념">

`infer` 키워드는 `extends` 키워드와 함께 조건부 타입에서 사용되며, 특정 위치의 타입을 추론해서 변수에 저장한 후 사용할 수 있게 해줍니다.
...
type ExtractApiData<T> = T extends { success: true; data: infer D }
  ? D
  : never;
